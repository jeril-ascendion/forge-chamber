...
import { getSkills, getProgressSessions, type SkillScores, type SessionProgress } from '../lib/api'
import SkillRadar from '../components/SkillRadar'
import XPBar from '../components/XPBar'

export default function Progress() {
  const navigate = useNavigate()
  const { engineerName, roleTrack, totalXp, totalSessions } = useAppStore()
  const [skills, setSkills] = useState<SkillScores>({ technical_depth: 0, communication: 0, debate_resilience: 0, ai_native: 0 })
                ...
                <tbody>
                  {sessions.map((s, i) => (
                    <tr
                      ...
                      onClick={() => navigate('/debrief', { state: { sessionId: s.date } })}
                    >
